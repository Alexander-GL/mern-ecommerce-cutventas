...
import { getCategories, isAuthenticated, createOther } from './apiCore';
...
const AddOther = () => {
    ...
    const showError = () => (
      ...
      const showSuccess = () => (
        <div className='alert alert-info' style={{ display: createdOther ? '' : 'none' }}>  
        <h5>{`${createdOther} ha sido creado correctamente`}</h5>
        </div>
      )
      ...
      const clickSubmit = event => {
        ...
        createOther(user._id, token, formData).then(data => {
          if (data.error) {
            setValues({ ...values, error: data.error })
          } else {
            setValues({
    ...
    return (
      <div>
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/css/bootstrap.min.css"
            ...
            <Navigation2 />
            <div className="container">
                  <img className="flayer-product-other" src={model_product} alt="Model Product" />
              <div className="row">
                <div className="col-md-12">
                <Jumbotron>
                  <h3 className="col-12">Otros Articulos</h3>
                    <div className="m-1">
                      {showLoading()}
                      {showSuccess()}
                      {showError()}
