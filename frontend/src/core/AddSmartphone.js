...
import { getCategories, isAuthenticated, createSmartphone } from './apiCore';
...
const AddSmartphone = () => {
    ...
    const showError = () => (
      ...
      const showSuccess = () => (
        <div className='alert alert-info' style={{ display: createdSmartphone ? '' : 'none' }}>  
        <h5>{`${createdSmartphone} ha sido creado correctamente`}</h5>
        </div>
      )
      ...
      const clickSubmit = event => {
        ...
        createSmartphone(user._id, token, formData).then(data => {
          if (data.error) {
            setValues({ ...values, error: data.error })
          } else {
            setValues({
      ...
      const newProductForm = () => (
        <div className="container">
          ...
          <div className='form-group'>
            <label className='text-muted'>Voltaje</label>
            <input
              onChange={handleChange('volts')}
              type='text'
              className='form-control'
              value={volts}
              required autoFocus
    ...
    return (
      <div>
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/css/bootstrap.min.css"
            ...
            <Navigation2 />
            <div className="container">
                  <img className="flayer-product-electronic" src={model_product} alt="Model Product" />
              <div className="row">
                <div className="col-md-12">
                <Jumbotron>
                  <h3 className="col-12">Smartphones o Telefonos inteligentes</h3>
                    <div className="m-1">
                      {showLoading()}
                      {showSuccess()}
                      {showError()}
