...
import { getCategories, isAuthenticated, createFit } from './apiCore';
...
const AddFit = () => {
    ...
    const showError = () => (
      ...
      const showSuccess = () => (
        <div className='alert alert-info' style={{ display: createdFit ? '' : 'none' }}>  
        <h5>{`${createdFit} ha sido creado correctamente`}</h5>
        </div>
      )
      ...
      const clickSubmit = event => {
        ...
        createFit(user._id, token, formData).then(data => {
          if (data.error) {
            setValues({ ...values, error: data.error })
          } else {
            setValues({
      ...
      <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.0/dist/css/bootstrap.min.css"
          ...
          <div className="container">
            ...
            <div className="row">
              ...
              <Jumbotron>
                <h3 className="col-12">Ropa Deportiva</h3>
                  <div className="m-1">
                    {showLoading()}
                    {showSuccess()}
                    {showError()}
